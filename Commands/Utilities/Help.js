...
module.exports = class extends Command {
    ...
    async run(message, [command]) {
        let e = new MessageEmbed()
            ...
            .setDescription(`Prefix in \`${message.guild.name}\` is \`${this.client.db.get(`prefix_${message.guild.id}`) ? this.client.db.get(`prefix_${message.guild.id}`) : this.client.config.prefix}\`
            Command Arguments:
            \`<>\` => Required
            \`[]\` => Optional`)
            .setTimestamp()
